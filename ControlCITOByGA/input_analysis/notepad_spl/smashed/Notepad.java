...
public class Notepad extends JFrame
{
	...
	public Notepad()
	{
		...
		/**
		 * initialize fields here.  For some reason, when you initialize
		 * fields outside of a method, they are null when constructMenu is
		 * called
		 */
		initializeFields();
		...
		if(NotepadTEST.SINGLETON.isUNDOREDO___())
		{
	...
	{
		/**
		 * Do the menu construction - be it toolbar and/or menubar - here.
		 * We need to do it before show().
		 */
		constructMenu();
	...
	{
		String path = System.getProperty("user.dir") + File.separator + "src" + File.separator + "images" + File.separator + filename;
	...
	{
		if (NotepadTEST.SINGLETON.isBASEMENUBAR___()) {
			constructMenuBar();
		}
		if (NotepadTEST.SINGLETON.isBASETOOLBAR___()) {
			constructToolBar();
		}   
	}
	/*@(BaseToolBar)*/
	...
	{
		if (NotepadTEST.SINGLETON.isUNDOREDOMENUBAR___()) {
			ediT.add(undoAction);
			ediT.add(redoAction);
		}
	...
	{
		if (NotepadTEST.SINGLETON.isUNDOREDOTOOLBAR___()) {
	...
	public void constructMenuBar()
	{
		...
		helP.add(abouT = new JMenuItem("About Notepad", 
		...
		if (NotepadTEST.SINGLETON.isEDITMENUBAR___()) {
			constructEditMenuBar();
		}

		if (NotepadTEST.SINGLETON.isFORMATMENUBAR___()) {
			Menubar.add(formaT = new JMenu("Format"));
			formaT.add(lineWraP = new JCheckBoxMenuItem("Line Wrap"));
			formaT.add(fonT = new JMenuItem("Font", 
					getImageIcon("font.gif")));
		...
		if (NotepadTEST.SINGLETON.isPERSISTENCEMENUBAR___()) {
			filE.add(opeN   = new JMenuItem
					("Open", getImageIcon("open.gif")));
			filE.add(savE   = new JMenuItem
					("Save", getImageIcon("save.gif")));
			filE.add(saveAS = new JMenuItem
					("Save As", getImageIcon("saveAs.gif")));
		...
		if (NotepadTEST.SINGLETON.isPRINTMENUBAR___()) {
			filE.add(prinT  = new JMenuItem("Print", 
					getImageIcon("print.gif")));
		...
		if (NotepadTEST.SINGLETON.isSEARCHMENUBAR___()) {
		...
		if (NotepadTEST.SINGLETON.isWORDCOUNTMENUBAR___()) {
			...
			wordCountMenuItem.setAccelerator(KeyStroke.getKeyStroke
	...
	public void constructToolBar()
	{
		...
		/**
		 *adding newButton, openButton, saveButton,
		  saveAsButton, printButton, undoButton,
		 *redoButton, cutButton, copyButton, pasteButton, 
		 fontButton & aboutButton to the tool bar,
		 *adding a small image icon to the menu item &
		 *adding separator between the button
		 */
		toolBar.add(newButton = new JButton
				(getImageIcon("new.gif")));
		toolBar.add(aboutButton = new JButton
				(getImageIcon("about.gif")));
		...
		if (NotepadTEST.SINGLETON.isEDITTOOLBAR___()) {
			toolBar.addSeparator();
			constructEditToolBar();
		}

		if (NotepadTEST.SINGLETON.isFORMATTOOLBAR___()) {
			toolBar.addSeparator();
			toolBar.add(fontButton  = new JButton
					(getImageIcon("font.gif")));
		...
		if (NotepadTEST.SINGLETON.isPERSISTENCETOOLBAR___()) {
			toolBar.addSeparator();
		...
		if (NotepadTEST.SINGLETON.isPRINTTOOLBAR___()) {
			toolBar.addSeparator();
			toolBar.add(printButton = new JButton(
					getImageIcon("print.gif")));
		...
		if (NotepadTEST.SINGLETON.isSEARCHTOOLBAR___()) {
			toolBar.addSeparator();
		...
		if (NotepadTEST.SINGLETON.isWORDCOUNTTOOLBAR___()) {
			toolBar.addSeparator();
			toolBar.add(wordCountButton = new JButton(
					getImageIcon("wordcount.gif")));
	...
	{
	/*	Config___.Base___ = true; 
		Config___.BaseMenuBar = true;
		Config___.BaseToolBar = true;
		Config___.Edit = true;
		Config___.EditMenuBar = true;
		Config___.EditToolBar = true;
		Config___.Format = true;
		Config___.FormatMenuBar = true;
		Config___.FormatToolBar = true;
		Config___.Persistence = true;
		Config___.PersistenceMenuBar = true;
		Config___.PersistenceToolBar = true;
		Config___.Print = true;
		Config___.PrintMenuBar = true;
		Config___.PrintToolBar = true;
		Config___.Search = true;
		Config___.SearchMenuBar = true;
		Config___.SearchToolBar = true;
		Config___.UndoRedo = true;
		Config___.UndoRedoMenuBar = true;
		Config___.UndoRedoToolBar = true;
		Config___.WordCount = true;
		Config___.WordCountMenuBar = true;
		Config___.WordCountToolBar = true; */
