...
public class NPopulations {

	private List<NChromosome> populations = new ArrayList<NChromosome>();; // 种群表示
	...
	public NPopulations(int populationSize) {
		this.POPNUM = populationSize;
	}

	public NPopulations(int populationSize, List<ClassInfo> listOfGA, boolean initialise) {
		this.POPNUM = populationSize;
		if (initialise) {
	...
	public void initialPops(int POPNUM, List<ClassInfo> listOfGA) {
		this.populations = new ArrayList<NChromosome>();
		for (int i = 0; i < POPNUM; i++) {
			NChromosome chromos = new NChromosome(listOfGA, false);
	 ...
	 * 每代种群进化
	 */
	public NPopulations evolvePops(NPopulations oldPopulations) {
		NPopulations newPopulations = new NPopulations(oldPopulations.getPOPNUM());
		// 根据适应值排序
		List<NChromosome> sortpopulations = sortPops(oldPopulations);
		// 选择操作保留上一代的优良个体
		if (elitism) {
		...
		for (int i = 0; i < crossnum / 2; i++) {
			...
			NChromosome parent1 = sortpopulations.get(a);
			NChromosome parent2 = sortpopulations.get(b);
			// 返回两个父个体交叉变异过的两个子个体
			List<NChromosome> children = crossover(parent1, parent2);
			// 将交叉变异的添加到新种群
		...
		for (int j = 0; j < mutatenum; j++) {
			...
			NChromosome parent = sortpopulations.get(a);
			NChromosome child = mutate(parent);
	 ...
	 * 交叉操作 交叉概率为pc
	 */
	public List<NChromosome> crossover(NChromosome parent1, NChromosome parent2) {

		// children返回生成的两个子代染色体
		List<NChromosome> children = new ArrayList<NChromosome>();
		// 随机生成交叉点
		Random random = new Random();
		...
		// 新染色体（个体）
		NChromosome child1 = new NChromosome(child1_list, true);
		...
		NChromosome child2 = new NChromosome(child2_list, true);
	 ...
	 * 变异操作 变异概率为pm
	 */
	public NChromosome mutate(NChromosome parent) {
		...
		NChromosome child = new NChromosome(child_list, true);
	 ...
	 */
	public NPopulations FullOrEmpty(NPopulations oldPopulations, NPopulations newPopulations) {
		//删除出重复
		List<NChromosome> list=newPopulations.getPopulations();
	 ...
	 */
	public List<NChromosome> sortPops(NPopulations population) {
		List<NChromosome> sortpopulations = population.getPopulations();
	 ...
	 */
	public NChromosome getBest(NPopulations population) {
		NChromosome bestchromos = null;
		List<NChromosome> sortpopulations = sortPops(population);
	 ...
	 * 寻找最差个体，并将其记录下来
	 */
	public NChromosome getWorst(NPopulations population) {
		NChromosome worstchromos = null;
		List<NChromosome> sortpopulations = sortPops(population);
		worstchromos = sortpopulations.get(getPOPNUM() - 1);
		return worstchromos;
	}

	public List<NChromosome> getPopulations() {
		return populations;
	}

	public void setPopulations(List<NChromosome> populations) {
		this.populations = populations;
	}

	public void initOcplxAndStubAndWoAndFit() {
		for (NChromosome chromosome : this.getPopulations()) {
			...
			if (depClasses.size() != 0) {
				for (String depClass : depClasses) {
					...
					if (!classNames.contains(depClass)) {
						...
						double complex = NewSCplx.calculateSCplxByEntropy(order, sci, scd);
